...
public class UserServiceImpl implements UserService  {
     ...
     */
    @Override
    public UserModel updatePwd(String telphone, String encryptPassword) throws BusinessException {
        // 通过用户的手机获取用户信息
        /* 这里的selectByTelphone是我们在UserDOMapper.xml中手动实现的 */
        UserDO userDO = userDOMapper.selectByTelephone(telphone);
        if (userDO == null){
            throw new BusinessException(EmBusinessError.UPDATE_PWD_FAIL);
        }

        UserPasswordDO userPasswordDO = userPasswordDOMapper.selectByUserId(userDO.getId());
        userPasswordDO.setEncryptPassword(encryptPassword);
